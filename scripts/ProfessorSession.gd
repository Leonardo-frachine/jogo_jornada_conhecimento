...
# Estado local da sessao do professor e da sala atualmente selecionada no painel.
...
func clear_session() -> void:
	# Logout remove identidade e contexto da sala para impedir vazamento entre professores.
	professor_id = 0
	professor_name = ""
	professor_email = ""
	current_room_id = 0
...
func start_session(professor_payload: Dictionary) -> void:
	# Inicia com dados publicos da API e sem herdar a sala de uma sessao anterior.
...
func has_session() -> bool:
	# IDs positivos representam professores persistidos e autenticados.
	return professor_id > 0

func set_current_room(room_payload: Dictionary) -> void:
	# Guarda somente os metadados necessarios para as demais telas e requisicoes.
...
func has_current_room() -> bool:
	# A sala atual e valida quando possui identificador persistido.
